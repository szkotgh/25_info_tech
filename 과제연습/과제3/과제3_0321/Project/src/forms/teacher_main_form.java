...
public class teacher_main_form extends aframe{
	
	JButton jb[] = new JButton[2];
	String bn[] = {"질문 목록", "통계"};
	
	public teacher_main_form() {
		fs("선생님메인");
		emp(pc, 0, 5, 5, 20);
		...
		for (int i=0; i<bn.length; i++) {
			...
			jb[i].addActionListener(e -> {
				if (a == 0) {
					new teacher_question_list_form();
				} else {
					new teacher_statistics_form();
				}
				dispose();
			});
		}
	...
	private void dup() {
		jp.removeAll();
		try {
			rs = db.rs("SELECT c.cno,c.uno,c.title,u.name,u.grade,c.explan FROM catalog c,teacher t,user u where c.uno = u.uno and c.tno = t.tno and c.tno = " + vq.uno);
					...
					img.addMouseListener(new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent e) {
							if (e.getClickCount() == 2) {
								dispose();
								vq.cno=cno;
									new student_question_solve_form();
							}
						}
					});
